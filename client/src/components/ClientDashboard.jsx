  ...
  const EnhancedSpaceCard = ({ space }) => {
    ...
    return (
      <div className="border w-full h-full rounded-lg overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col">
        <div className="relative w-full h-48 sm:h-60 overflow-hidden bg-gray-200">
          {imageUrl && !imageError ? (
            <>
              <img
                src={imageUrl}
          ...
          {(!imageUrl || imageError) && (
            <div className="absolute inset-0 bg-gray-200 flex items-center justify-center text-gray-500">
              <div className="text-center">
                <Building className="h-10 w-10 sm:h-12 sm:w-12 mx-auto mb-2" />
        ...
        <div className="p-3 sm:p-4 flex flex-col flex-grow">
          <h3 className="text-base sm:text-lg font-semibold mb-2 line-clamp-1">{space.title || space.name || 'Untitled Space'}</h3>
          <p className="text-gray-600 text-xs sm:text-sm mb-3 line-clamp-2 flex-grow">{space.description || 'No description available'}</p>

          <div className="mt-auto space-y-3">
            <div className="flex justify-between items-center text-xs sm:text-sm">
              <span className="text-gray-600 capitalize">{space.space_type || 'General'}</span>
              {space.location && (
                <span className="text-gray-500 flex items-center">
                  <MapPin className="h-3 w-3 mr-1" />
                  <span className="truncate max-w-[100px]">{space.location}</span>
                </span>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                className="bg-gray-200 text-gray-800 px-3 sm:px-4 py-2 rounded-lg hover:bg-gray-300 text-xs sm:text-sm transition-colors"
              >
                View Details
              </button>
              ...
              {space.status === "available" ? (
                <Link to={`/spaces/${space.id}/booking`}>
                  <button className="w-full bg-blue-500 text-white px-3 sm:px-4 py-2 rounded-lg hover:bg-blue-600 text-xs sm:text-sm transition-colors">
                    Book Now
                  </button>
                </Link>
              ) : (
                <button
                  disabled
                  className="w-full bg-gray-400 text-gray-200 px-3 sm:px-4 py-2 rounded-lg cursor-not-allowed text-xs sm:text-sm flex items-center justify-center"
                >
                  <CalendarOff className="h-3 w-3 sm:h-4 sm:w-4 mr-1" />
                  Currently Booked
                </button>
              )}
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center py-4 sm:py-6 space-y-2 sm:space-y-0">
            <div>
              ...
              <p className="text-sm sm:text-base text-gray-600">Welcome back! Browse all spaces and manage your bookings.</p>
            </div>
        ...
        {activeTab === 'dashboard' && (
          <div className="space-y-6 sm:space-y-8">
            ...
            <div className="bg-white rounded-lg shadow-md">
              <div className="p-4 sm:p-6 border-b flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-2 sm:space-y-0">
                <h3 className="text-base sm:text-lg font-semibold text-gray-900">Upcoming Bookings</h3>
                <button
                ...
                {upcomingBookings.length > 0 ? (
                  <div className="space-y-4">
                    {upcomingBookings.map(booking => (
                      <div key={booking.id} className="border rounded-lg p-3 sm:p-4 hover:shadow-md transition-shadow">
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 space-y-2 sm:space-y-0">
                          <div>
                            ...
                            <p className="text-xs sm:text-sm text-gray-600">Booking #{booking.id}</p>
                          </div>
                        ...
                        <div className="flex flex-col sm:flex-row justify-end mt-4 space-y-2 sm:space-y-0 sm:space-x-2">
                          <Link
                            ...
                            className="px-3 sm:px-4 py-2 text-xs sm:text-sm bg-gray-100 text-gray-700 rounded hover:bg-gray-200 text-center"
                          >
                            View Details
                          </Link>
                          {(booking.status === 'pending' || booking.status === 'upcoming') && (
                            <button className="px-3 sm:px-4 py-2 text-xs sm:text-sm bg-red-100 text-red-700 rounded hover:bg-red-200">
                              Cancel
                            </button>
                          )}
                        </div>
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500 text-sm sm:text-base">No upcoming bookings</p>
                    <button
            ...
            <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                ...
                >
                  ...
                  <span className="text-xs sm:text-sm font-medium text-blue-900">Browse Spaces</span>
                </button>
                <button
                  onClick={() => setActiveTab('bookings')}
                ...
                >
                  ...
                  <span className="text-xs sm:text-sm font-medium text-green-900">My Bookings</span>
                </button>
                <button
                  onClick={() => setActiveTab('invoices')}
                  className="p-3 sm:p-4 bg-purple-50 rounded-lg text-center hover:bg-purple-100 transition-colors"
                >
                  <CreditCard className="h-6 w-6 sm:h-8 sm:w-8 text-purple-600 mx-auto mb-2" />
                  <span className="text-xs sm:text-sm font-medium text-purple-900">Payments</span>
                </button>
                <button
                  onClick={() => setActiveTab('reviews')}
                  className="p-3 sm:p-4 bg-orange-50 rounded-lg text-center hover:bg-orange-100 transition-colors"
                >
                  <Star className="h-6 w-6 sm:h-8 sm:w-8 text-orange-600 mx-auto mb-2" />
                  <span className="text-xs sm:text-sm font-medium text-orange-900">Write Reviews</span>
                </button>
              </div>
            </div>
          </div>
        ...
        {activeTab === 'browse' && (
          <div className="space-y-4 sm:space-y-6">
            ...
            <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
                ...
                <div className="flex-1">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="pl-10 pr-4 py-2 w-full text-sm sm:text-base border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    />
                  </div>
                </div>
              ...
              {/* Results summary */}
              <div className="mt-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 text-xs sm:text-sm text-gray-600">
                <span>
          ...
          <div className="space-y-4 sm:space-y-6">
            <div className="bg-white rounded-lg shadow-md">
              <div className="p-4 sm:p-6 border-b">
                <h3 className="text-base sm:text-lg font-semibold text-gray-900">Review Management</h3>
                <p className="text-gray-600 text-xs sm:text-sm mt-1">Write reviews for completed bookings</p>
              </div>
              <div className="p-4 sm:p-6">
                {clientStats.pendingReviews > 0 ? (
                  <div className="space-y-4 mb-8">
                    <h4 className="font-medium text-gray-900 text-sm sm:text-base">Pending Reviews</h4>
                    {bookings.filter(b => {
                      const endTime = b.end_time || b.completed_date;
                      return endTime && new Date(endTime) < new Date() &&
                        (b.status === 'completed' || b.status === 'confirmed') &&
                      ...
                      <div key={booking.id} className="border rounded-lg p-3 sm:p-4">
                        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-3 space-y-2 sm:space-y-0">
                          ...
                          <span className="text-xs sm:text-sm text-orange-600 font-medium self-start">Review Pending</span>
                        </div>
                        <Link
                          to={`/spaces/${booking.space_id}`}
                          className="bg-blue-600 text-white px-3 sm:px-4 py-2 rounded-lg hover:bg-blue-700 inline-block text-xs sm:text-sm"
                        >
                          Write Review
                        </Link>
                      </div>
                ...
                {reviews.length > 0 && (
                  <div>
                    <h4 className="font-medium text-gray-900 mb-4 text-sm sm:text-base">My Reviews</h4>
                    <div className="space-y-4">
                      {reviews.map(review => (
                        <div key={review.id} className="border rounded-lg p-3 sm:p-4">
                          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-2 space-y-2 sm:space-y-0">
                            <div>
                              <h5 className="font-medium text-gray-900 text-sm sm:text-base">Booking #{review.booking_id}</h5>
                            ...
                            <span className="text-xs sm:text-sm text-gray-500 self-start">
                ...
                {clientStats.pendingReviews === 0 && reviews.length === 0 && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-gray-500 text-sm sm:text-base">No reviews to show yet</p>
                    <p className="text-xs sm:text-sm text-gray-400 mt-2">Complete a booking to leave a review</p>
                  </div>
                )}
              </div>
            </div>
